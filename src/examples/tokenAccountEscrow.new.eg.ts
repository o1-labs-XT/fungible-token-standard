...
export class TokenAccountEscrow extends SmartContract {
  ...
  @method async deposit(
    ...
    const senderTokenAccount = AccountUpdate.createSigned(sender, this.tokenId);
    ...
    const currentBalance = senderTokenAccount.update.appState[0].value;
    senderTokenAccount.update.appState[0].value = currentBalance.add(
      amount.value
    );

    await token.mint(sender, amount, proof, vk, vKeyMap);
  }
  ...
  @method async withdraw(
    ...
    const sender = this.sender.getUnconstrained();
    const userTokenAccount = AccountUpdate.createSigned(sender, this.tokenId);
    userTokenAccount.body.mayUseToken =
      AccountUpdate.MayUseToken.InheritFromParent;
    ...
    await token.burn(sender, amount, proof, vk, vKeyMap);
  }
}
